...
Warning: This script is only meant to be run hourly, as F@H has explicitly
  ...
  An IP ban could result otherwise.
...
# TODO(justinlulejian): Add ability to get contents from local file (testing).
def retrieve_userdata_file_content(user_data_url):
  ...
  except req_exc.ConnectionError as error:
    logging.fatal(
        'Was unable to connect to: %s. Please check your internet connection '
        'and try again. Error: %s.', user_data_url, error)
    sys.exit()
  except (req_exc.MissingSchema, req_exc.InvalidURL) as error:
    logging.fatal(
        'The URL specified: %s does not appear valid. Error: %s',
        user_data_url, error)
    sys.exit()
  except req_exc.BaseHTTPError as error:
    logging.fatal(
...
def decompress_userdata_to_list(user_data_content):
  ...
  try:
    ...
    logging.fatal('Couldn\'t decompress empty file. Error: %s', error)
    sys.exit()
...
def validate_record_location(ctx, param, value):  # pylint: disable=unused-argument
  ...
  try:
    ...
    raise click.BadParameter(
        'Couldn\'t create or access %s. IOError #: %s', value, error.errno)
  return value
...
def record_folding_at_home_stats(
  ...
  Args:
  record_file: A string of a path to a desired filename to record the CSV
    of F@H user data. File will be created if it doesn't exist.
  userdata_location: A string of the URL or filepath where the bz2 file
    containing F@H user data is located.
  username: A string of the F@H donator username.
  verbose: A boolean to show debug and above logging.
  """
  setup_cli_logging(verbose)
  logging.debug('Starting retrieval of user data.')
  all_user_data = (
      decompress_userdata_to_list(
          retrieve_userdata_file_content(userdata_location)))
...
@click.option(
    ...
    help='Where to retrieve bz2 user data file.', show_default=True)
@click.option(
    '--username',
    default='', help='The donator username to get stats for. Case-sensitive.',
    callback=validate_fah_username, show_default=True)
@click.option(
    '--verbose',
    default=False,
    help='Show more verbose logging output.', show_default=True)
def record_folding_stats_cli(
    record_file, userdata_location, username, verbose):
  """CLI interface for folding_stats_recorder."""
  record_folding_at_home_stats(
      record_file, userdata_location, username, verbose)
